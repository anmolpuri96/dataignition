...
def compare_text(overlap_threshold=0.6):
    ...
    for category in unanswered_redis.scan_iter('cat:*'):
        ...
        if answered_members:
            ...
            unanswered_minhash = ids_df.withColumn("unanswered_minhash", F.lit("UnansweredId"))
            answered_minhash = unanswered_minhash.withColumn("answered_minhash", F.lit("AnsweredId"))
